...
const props = defineProps({
  pagination: {
    ...
    required: true,
    validator: (value) => {
      return ['current_page', 'last_page', 'per_page', 'total'].every(key =>
...
defineEmits(['pageChange', 'previous', 'next'])
...
function pageButtonClass(page) {
  ...
  if (page === props.pagination.current_page) {
    return `${baseClass} z-10 bg-primary-50 border-primary-500 text-primary-600`
  }
